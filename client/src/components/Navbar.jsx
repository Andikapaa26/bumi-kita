    ...
    <header className=" bg-white md:bg-transparent fixed top-0 left-0 right-0">
      <nav className={`py-4 lg:px-14 px-4 ${
        ...
        <div className="flex justify-between items-center text-base gap-8">
          <a href="" className="text-2xl font-semibold flex items-center space-x-3"><img src={logo} alt="" className="w-10 inline-block items-center"/><span>Bumi Kita</span></a>

          <ul className="md:flex space-x-12 hidden">
            
            {
                navItems.map(({ link, path }) => <a href={path} spy={true} smooth={true} offset={-100} key={path} className='block text-base text-neutralDGrey hover:text-brandPrimary first:font-medium'>{link}</a>)
            }
          </ul>
